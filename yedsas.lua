...
function build_aliases()
    ...
    Aliases[".k"] = "knock"; Aliases.k = "knock"; Aliases["rj!"] = "rejoin"; Aliases.lock = "target"
...
RunService.Heartbeat:Connect(function()
    pcall(function()
        ...
        if Attacking then if Config.GunMode and Config.Melee == "Punch" then GunAttack() else Attack() end end
        ...
        if Boxing and OwnerCharacter then Remotes.Melee:InvokeServer("Melee", "Punch", OwnerCharacter.HumanoidRootPart.CFrame * CFrame.new(0,0,-5), nil) end

        if OwnerCharacter and CurrentOwner and CurrentOwner.Character and Config.Position and Config.Position ~= "Stop" then
            local posOffset
            if Config.Position == "Target" and targetChar then
                posOffset = targetChar.HumanoidRootPart.CFrame
            else
